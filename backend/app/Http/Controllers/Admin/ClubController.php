...
{
    /**
     * Display a listing of club activities
     */
    public function index(Request $request)
    {
        ...
        $clubs = $query->with('admin')->orderBy('date', 'desc')->get();
    ...
    /**
     * Store a newly created club activity
     */
    public function store(Request $request)
    {
        ...
        $club = Club::create([
            ...
            'category' => $request->category ?? 'Club',
            'date' => $request->date,
            'time' => $request->time,
        ...
        return response()->json([
            ...
            'data' => [
                'club' => $club->load('admin'),
            ],
        ], 201);
    }

    /**
     * Display the specified club activity
     */
    public function show($id)
    {
        $admin = Auth::guard('admin')->user();
        
        $club = Club::with(['admin', 'inscriptions.user'])->findOrFail($id);

        // Role-based access check
        if (!$admin->is_super_admin && $club->admin_id !== $admin->id) {
            return response()->json([
                'success' => false,
    ...
    /**
     * Update the specified club activity
     */
    public function update(Request $request, $id)
    {
        ...
        if (!$admin->is_super_admin && $club->admin_id !== $admin->id) {
            return response()->json([
                'success' => false,
                'message' => 'Vous ne pouvez modifier que vos propres clubs',
            ], 403);
        }
    ...
    /**
     * Remove the specified club activity
     */
    public function destroy($id)
    {
        ...
        if (!$admin->is_super_admin && $club->admin_id !== $admin->id) {
            return response()->json([
                'success' => false,
                'message' => 'Vous ne pouvez supprimer que vos propres clubs',
            ], 403);
        }
